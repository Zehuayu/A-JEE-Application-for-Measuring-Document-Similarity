...
import ie.gmit.sw.similarity.SimilarityController;
 ...
 */
@WebServlet(urlPatterns = "/doProcess", asyncSupported = true)
...
public class ServiceHandler extends HttpServlet {
	...
	public void init() throws ServletException {
		...
		sizeOfShingle = Integer.parseInt(ctx.getInitParameter("SIZE_OF_SHINGLE"));
	}
	...
	public void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		if (WEBINF_PATH == null) {
			WEBINF_PATH = req.getServletContext().getRealPath("/WEB-INF");
		}
...
//		jobNumber = TaskNumber.getJobNumber();
		if (taskNumber == null) {
...
//		
		...
		String uploadFileFolder = req.getServletContext().getRealPath("/WEB-INF/files");
		String uploadFile = uploadFileFolder + "/" + new Random().nextInt();
		...
		SimilarityController controller = new SimilarityController(asyncCtx);
		controller.setTaskNumber(taskNumber);
		controller.setSizeOfShingle(sizeOfShingle);
		controller.setUploadFile(uploadFile);
		controller.setTitle(title);
		
		new Thread(controller).start();
		
//		controller.execute(taskNumber, sizeOfShingle, uploadFile, title);
